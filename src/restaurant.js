...
const somaDosPrecosDosPedidos = () => {
  const comida = restaurant.food;
  const bebidas = restaurant.drink;

  let total = 0;
  for (let index = 0; index < restaurant.consumption.length; index += 1) {
    const element = restaurant.consumption[index];
    
    if (comida[element] !== undefined) {
      total += comida[element];
    }

    if (bebidas[element] !== undefined) {
      total += bebidas[element];
    }
  }
